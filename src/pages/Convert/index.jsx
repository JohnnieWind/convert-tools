...
function Convert(props) {
  ...
  const [type, setType] = React.useState({ name: '', description: '', accept: '', exportType: '', })
  ...
  const handleConvert = async () => {
    ...
    const intervalFetch = setInterval(async () => {
      ...
      const { result, code } = json1

      if (result === 'fail') {

        if (code === 2) {
          setData([
            { ...convertFile, response, status: '转换失败' }
          ])
          // ip超限
        ...
        response.taskId = '1131635493618064296.txt'
        setData([
          { ...convertFile, response, status: '转换中' }
        ])

      } else if (result === 'success') {
        setData([
          { ...convertFile, percent: 100, status: '转换完成' }
        ])
        clearInterval(intervalFetch)
      }
